$.namespace("sitePopGrid.eventhandler");
sitePopGrid.eventhandler = {
    // 초기화
    init : function () {
        this.gridLoading();
    ...
    ,gridLoading : function () {
        ...
        //그리드 데이터 세팅
        this.controller.callbackForCommonSuccess(this.grid, _siteList);
    ...
    ,apply : function (checkedRowNum) {
        ...
        var returnRow = this.grid.getDataSource().getJsonRow(checkedRowNum);
        window.postMessage(JSON.stringify(returnRow), _baseUrl);
        this.close();
    }
    ,gridEvent : {
