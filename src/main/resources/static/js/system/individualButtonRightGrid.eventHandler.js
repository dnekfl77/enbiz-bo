$.namespace("individualButtonRightGrid.eventhandler");
individualButtonRightGrid.eventhandler = {
	...
	bindButtonEvent: function() {
		...
		$('#btn_excel_down2').on('click', function(){
	...
	onSave: function() {
		...
		if (!result) {
			alert(col.noCheckedIndividualButtonRight);
			return;
		}
	...
	treeNodeClick : function(treeNode){
        var pin = {
				sysGbCd : treeNode.sysGbCd
            ,	rtTgtSeq : treeNode.rtTgtSeq
			,	rtGrpNo : treeNode.rtGrpNo
        }
		this.sysGbCd = treeNode.sysGbCd;
		this.rtTgtSeq = treeNode.rtTgtSeq;
		this.onSearch(0);
    }
};