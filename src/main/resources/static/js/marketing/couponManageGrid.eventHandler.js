$.namespace("couponManageGrid.eventhandler");
couponManageGrid.eventhandler = {
    // 초기화
    init : function () {
        this.calendarInit();
        this.gridLoading();
    ...
    bindButtonEvent : function(){
        ...
        this.grid.onCellDblClicked = function(grid,clickData){
            ...
            if(clickData.column==='promoNo'|| clickData.column==='promoNm'){
                self.callCouponRegistPopup(currentData.promoNo);
            }
        }
        // 초기화
        $('#btn_init').click(function() {
            $('#couponManageGridForm')[0].reset();
            $('#userId').val('');
            self.calendarInit();
        });
        ...
        // 등록자 팝업 버튼
        $('#btn_coupon_register').click(function() {
            self.callUserPopup();
        });

        // 등록 버튼
        $('#btn_grid_insert').click(function(){
            self.callCouponRegistPopup();
        })
        ...
        $("#couponManageGridForm").keypress(function (e){
            if (e.which == 13){
                $('#btn_list').click();
    ...
    popupCouponRegistCallback : function(e){
        if(e.data === 'succeeded'){
            couponManageGrid.eventhandler.onSearch(0);
        }
    }
};
