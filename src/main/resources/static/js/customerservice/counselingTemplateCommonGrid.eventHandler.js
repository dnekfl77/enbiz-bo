$.namespace("counselingTemplateCommonGrid.eventhandler");
counselingTemplateCommonGrid.eventhandler = {
    // 초기화
    init : function () {
        this.gridLoading();
    ...
    bindButtonEvent : function(){
        ...
        // 검색조건 초기화 버튼
        $("#btn_common_init").click(function() {
            $('#counselingTemplateCommonForm')[0].reset();
            $('#btn_common_reset_user_popup').click();
            self.valueInit();
        });

        // 검색조건 조회 버튼
        $("#btn_common_list").click(function() {
            self.onSearch(0,true);
        });

        // form 엔터키 입력
        $("#counselingTemplateCommonForm").keypress(function (e){
            if (e.which == 13){
                $('#btn_common_list').click();
                window.event.returnValue=false;
            }
        });

        // 그리드 템플릿 추가 버튼
        $("#btn_common_grid_insert").click(function() {
            self.onAdd("I", null, null);
        });

        // 그리드 행삭제 버튼
        $("#btn_common_grid_remove").click(function() {
            self.grid.cancel();
            self.onDelete();
        });

        // 그리드 변경 취소 버튼
        $("#btn_common_grid_reset").click(function() {
            self.onReset();
        });

        // 그리드 저장 버튼
        $("#btn_common_grid_save").click(function() {
            self.onSave();
        });

		// 조회 조건 : 등록자 검색버튼 클릭시
		$('#btn_common_call_user_popup').click(function () {
		    self.onUserPopup();
		});

		// 조회 조건 : 등록자 지우개버튼 클릭시
		$('#btn_common_reset_user_popup').click(function () {
			$('#common_userId').val('');
			$('#common_userNm').val('');
		});
    },
    ...
    onUserPopup : function(){
        ...
        popCommon(pin, POP_DATA, function (e) {
             ...
             $('#common_userId').val(returnValue[0].userId);
             $('#common_userNm').val(returnValue[0].userNm);
        });
    },
    ...
    onAdd : function(state, cnslAempId, cnslTmplNo) {
        var self = this;
        var pin = { argInsertUpdate : state, cnslAempId : cnslAempId, cnslTmplNo : cnslTmplNo, typeCode : "common" };
        if(state == "I"){ // 템플릿 추가
            var POP_DATA = {
