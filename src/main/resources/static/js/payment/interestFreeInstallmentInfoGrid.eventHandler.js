$.namespace("interestFreeInstallmentInfoGrid.eventhandler");
interestFreeInstallmentInfoGrid.eventhandler = {
    // 초기화
    init : function () {
        this.calendarInit();
        this.gridLoading();
    ...
    gridLoading : function(){
        ...
        this.grid.onCellClicked = function(grid,clickData){
            ...
            if(clickData.cellType != "header") {
                if(clickData.column==='nintInstGdNo'){
                    self.updatePopup("U",currentData.nintInstGdNo);
                }
    ...
    bindButtonEvent : function(){
        ...
        $('#btn_init').click(function() {
            $('#interestFreeInstallmentInfoGridForm')[0].reset();
            self.calendarInit();
        });

        $("#interestFreeInstallmentInfoGridForm").keypress(function (e){
            if (e.which == 13){
                $('#btn_list').click();
    ...
    updatePopup : function(state, nintInstGdNo){
        var self = this;
        if(state == "I"){
            var pin = { argInsertUpdate : state };
            var POP_DATA = {
                url: _baseUrl + "payment/NoInterestInstallmentGuideMgmt.noInterestInstallmentGuideSaveView.do"
                , winName: 'getInterestFreeInstallmentInfoInsert'
                , title: '무이자 할부 안내 등록'
                , _title: '무이자 할부 안내 등록'
                , left: 20
                , top: 40
            ...
            var pin = { argInsertUpdate : state, nintInstGdNo : nintInstGdNo };
            var POP_DATA = {
                url: _baseUrl + "payment/NoInterestInstallmentGuideMgmt.noInterestInstallmentGuideSaveView.do"
                , winName: 'getInterestFreeInstallmentInfoUpdate'
                , title: '무이자 할부 안내 수정'
                , _title: '무이자 할부 안내 수정'
                , left: 20
                , top: 40
