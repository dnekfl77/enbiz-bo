$.namespace('customerSearchResultGrid.eventhandler');
customerSearchResultGrid.eventhandler = {
    validationBySearch : function () {
        var bValidationBySearch = true;

        if($('#condxSysRegDtmAll').prop('checked') == true) {
            if ( $('#condxMbrValue').val().length <= 0 &&
                        $('#condxCellNo').val().length <= 0 &&
                        $('#condxTelNo').val().length <= 0 ) {
                alert('전체기간으로 조회시 회원정보, 휴대폰번호, 전화번호중 조회조건을 하나이상 입력하세요.');
                return false;
            }
        } else {
    ...
    // 검색 조건 초기화
    ,onReset : function () {
        $('#customerSearchGridForm')[0].reset();
    }
    // 회원 목록 조회
    ...
    ,gridEvent : {
        ...
        ,onCellDblClicked : function(grid, clickData) {
            ...
            if(clickData.column==='mbrNo'|| clickData.column==='mbrNm'){
                customerSearchResultGrid.eventhandler.popupCustomerDetail(currentData.mbrNo);
            }
        }
