var packageDetailInfoMsg = x2coMessage.getMessage({
    maxImageCntMsg        : 'packageGoods.detailInfo.alert.msg.maxImageCntMsg'
    ,invalidImageTypeMsg   : 'packageGoods.detailInfo.alert.msg.invalidImageTypeMsg'
    ,maxImageSizeMsg       : 'packageGoods.detailInfo.alert.msg.maxImageSizeMsg'
    ,maxTotalImageSizeMsg  : 'packageGoods.detailInfo.alert.msg.maxTotalImageSizeMsg'
    ,minImageCntMsg        : 'packageGoods.detailInfo.alert.msg.minImageCntMsg'
    ,noDetailContentsMsg   : 'packageGoods.detailInfo.alert.msg.noDetailContentsMsg'
    ,noImageReplaceTextMsg : 'packageGoods.detailInfo.alert.msg.noImageReplaceTextMsg'
});

$.namespace('packageDetailInfo.eventhandler');
packageDetailInfo.eventhandler = {
    // 초기화
    init : function () {
        this.setting();
    ...
    ,setting : function () {
        ...
        packageDetailInfo.eventhandler.oEditors = oEditors;
    }
    // 이벤트
    ,bindButtonEvent : function () {
        ...
        $('#btn_add_detail_image').click( function () {
            ...
            if( imageCnt === 10 ) {
                alert(packageDetailInfoMsg.maxImageCntMsg);
                return;
            }
            ...
            newDetailImage.find('.imageFile').bind('change', function (event) {
                ...
                if($.inArray(extension,['jpg']) === -1){
                    alert(packageDetailInfoMsg.invalidImageTypeMsg);
                    return;
                }

                // 개별 이미지 사이즈 확인
                if ( imageSize > that.maxImgSize ) {
                    alert(packageDetailInfoMsg.maxImageSizeMsg);
                    return;
                }
                ...
                if( that.maxTotalImgSize < totalImageSize ){
                    alert(packageDetailInfoMsg.maxTotalImageSizeMsg);
                    return;
                }
        ...
        $('#btn_remove_detail_image').click( function () {
            ...
            var imageCnt = $('tr[type=newDetailImage]').length;
            if( imageCnt === 1 ) {
                alert(packageDetailInfoMsg.minImageCntMsg);
                return;
            }
    ...
    ,resetDetailInfo : function () {
        ...
        // 웹에디터 초기화
        var oEditors = packageDetailInfo.eventhandler.oEditors;
    ...
    ,validationCheck : function () {
        ...
        if ( $('#detailInfo :radio[name=detailType]:checked').val() === '01' ) {
            ...
            if( editorContents == ""  || editorContents == null || editorContents == '&nbsp;' || editorContents == '<p>&nbsp;</p>')  {
                alert(packageDetailInfoMsg.noDetailContentsMsg);
        ...
        // 상품상세등록타입 : 이미지
            ...
            $.each( imageList, function ( index, image ) {
                ...
                if ( fileName !== '' &&replaceText === '' ) {
                    alert(packageDetailInfoMsg.noImageReplaceTextMsg);
