var imageInfoMsg = x2coMessage.getMessage({
...
$.namespace('imageInfo.eventhandler')
imageInfo.eventhandler = {
    // 초기화
    init : function () {
    ...
    ,bindButtonEvent : function () {
        ...
        $('#baseImageFile').change(function (event) {
            ...
            if($.inArray(extension,['jpg']) === -1){
                alert(imageInfoMsg.invalidImageTypeMsg);
                return;
            }

            // 개별 이미지 사이즈 확인
            if ( imageSize > that.maxImgSize ) {
                alert(imageInfoMsg.maxBaseImageSizeMsg);
                return;
            }
            ...
            if( that.maxTotalImgSize < totalImageSize ){
                alert(imageInfoMsg.maxTotalImageSizeMsg);
                return;
            }
        ...
        $('#btn_add_sub_image').click(function () {
            ...
            if( imageCnt === 10 ) {
                alert(imageInfoMsg.maxSubImageCntMsg);
                return;
            }
            ...
            newSubImage.find('.imageFile').bind('change', function (event) {
                ...
                // 개별 이미지 사이즈 확인
                if ( imageSize > that.maxImgSize ) {
                    alert(imageInfoMsg.maxSubImageSizeMsg);
                    return;
                }
        ...
        $('#btn_remove_sub_image').click( function () {
            ...
            var imageCnt = $('tr[type=newSubImage]').length;
            if( imageCnt === 0 ) {
                alert(imageInfoMsg.noImageForRemoveMsg);
                return;
            }
    ...
    ,validationCheck : function () {
        ...
        if ( $('#baseImageFileName').val() === '' ) {
            alert(imageInfoMsg.noBaseImageMsg);
