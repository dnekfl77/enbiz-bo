var assocGoodsGridMsg = x2coMessage.getMessage({
...
$.namespace('assocGoodsGrid.eventhandler')
assocGoodsGrid.eventhandler = {
    // 초기화
    init : function () {
        this.gridSetting();
    ...
    ,bindButtonEvent : function () {
        ...
        // 행추가 버튼 클릭
        $('#btn_assocGoodsGrid_add').click( function () {
            that.addAssociatedGoods();
        });

        // 행삭제 버큰 클릭
        $('#btn_assocGoodsGrid_remove').click( function () {
            that.removeAssociatedGoods();
        });

        // 변경취소 버큰 클릭
        $('#btn_assocGoodsGrid_reset').click( function () {
    ...
    ,addAssociatedGoods : function () {
        ...
        popCommon(pin, POP_DATA, function (e) {
            ...
            if ( that.isExistAssociatedGoods( goodsNo ) ) {
                alert(assocGoodsGridMsg.duplicatedAssocGoodsMsg);
                return;
            }
    ...
    ,removeAssociatedGoods : function () {
        ...
        if (checkedRows.length == 0) {
            alert(assocGoodsGridMsg.noCheckedAssocGoodsForRemoveMsg);
            return;
        }
        ...
        onEditRowChanged : function (grid, itemIndex, dataRow, field, oldValue, newValue) {
            ...
            for( var i = 0 ; i < sortSeqList.length ; i++ ) {
                ...
                if ( sortSeqList[i] === newValue ) {
                    alert(assocGoodsGridMsg.duplicatedSorSeqMsg);
        ...
        ,onValidateColumn : function ( grid, column, inserting, value, itemIndex, dataRow ) {
            ...
            if ( rowState !== 'deleted') {
                if ( column.fieldName === "sortSeq" ) {
                    if ( value === undefined ) {
                        error.level = "error";
                        error.message = assocGoodsGridMsg.noSortSeqMsg;
                        return error;
                    }
                }
            }
    ...
    // 연관상품 세팅
    ,setAssocGoodsGrid : function ( gridData ) {
