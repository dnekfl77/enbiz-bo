var detailAdveWrdMsg = x2coMessage.getMessage({
...
$.namespace('detailAdveWrdGrid.eventhandler')
detailAdveWrdGrid.eventhandler = {
    // 초기화
    init : function () {
        this.gridSetting();
    ...
    ,bindButtonEvent : function () {
        ...
        // 행추가 버튼 클릭시
        $('#btn_detailAdveWrdGrid_add').click( function () {
            that.addAdvertisingWord();
        });

        // 행삭제 버튼 클릭시
        $('#btn_detailAdveWrdGrid_remove').click( function () {
    ...
    ,removeAdvertisingWord : function () {
        ...
        if (checkedRows.length == 0) {
            alert(detailAdveWrdMsg.noCheckedAdveWrdForRemoveMsg);
            return;
        }
        ...
        onEditRowChanged : function (grid, itemIndex, dataRow, field, oldValue, newValue) {
            ...
            if ( fieldName === 'aplyStrDt' ) {
                ...
                if( aplyStrDt === undefined || aplyStrDt === '' ){
                    alert(detailAdveWrdMsg.emptyApplyStartDateMsg);
                ...
                // 현재 날짜 이전 일자 선택 불가
                if( aplyStrDt < toDate ) {
                    alert(detailAdveWrdMsg.beforeTodayApplyStartDateMsg);
                    provider.setValue( dataRow, fieldName, oldValue );
                    return;
                }
                ...
                for ( var i = 0; i < aplyStrDtList.length ; i++ ) {
                    ...
                    if ( aplyStrDtList[i] === aplyStrDt ) {
                        alert(detailAdveWrdMsg.duplicatedAplyStrDtMsg);
                        provider.setValue( dataRow, fieldName, oldValue );
                        break;
                    }
                }
                ...
                if ( aplyStrDt > aplyEndDt ) {
                    alert(detailAdveWrdMsg.afterEndApplyStartDateMsg);
                    provider.setValue( dataRow, fieldName, oldValue );
                }
                ...
                if ( dateDiff > 30 ) {
                    alert(detailAdveWrdMsg.maxApplyPeriodMsg);
                    provider.setValue( dataRow, fieldName, oldValue );
                    return;
                }
            ...
            // 적용종료일자
                ...
                if( aplyEndDt === undefined || aplyEndDt === '' ){
                    alert(detailAdveWrdMsg.emptyApplyEndDateMsg);
                    provider.setValue( dataRow, fieldName, oldValue);
                    return;
                }
                ...
                // 현재 날짜 이전 일자 선택 불가
                if( aplyEndDt < toDate ) {
                    alert(detailAdveWrdMsg.beforeTodayApplyEndDateMsg);
                    provider.setValue( dataRow, fieldName, oldValue);
                    return;
                }
                ...
                for ( var i = 0; i < aplyEndDtList.length ; i++ ) {
                    ...
                    if ( aplyEndDtList[i] === aplyEndDt ) {
                        alert(detailAdveWrdMsg.duplicatedAplyEndDtMsg);
                        provider.setValue( dataRow, fieldName, oldValue );
                        break;
                    }
                }
                ...
                if ( newValue < aplyStrDt ) {
                    alert(detailAdveWrdMsg.beforeStartApplyEndDateMsg);
                    provider.setValue( dataRow, fieldName, oldValue );
                }
            ...
            // 사용여부 체크시
                ...
                if ( thisAplyStrDt === '' || thisAplyEndDt === '' ) {
                    alert(detailAdveWrdMsg.checkApplyDateBeforeUseMsg);
                ...
                for ( var i = 0; i < aplyStrDtList.length ; i++ ) {
                    ...
                    alert(detailAdveWrdMsg.duplicatedPeriodAdveWrdMsg);
                    provider.setValue( dataRow, 'useYn', 'N' );
                    break;
                }
            }
        ...
        ,onValidateColumn : function ( grid, column, inserting, value, itemIndex, dataRow ) {
            ...
            if ( column.fieldName === 'adveWrd' ) {
                ...
                if ( value === undefined || value === '' ) {
                    error.level = 'error';
                    error.message = detailAdveWrdMsg.noAdveWrdMsg;
                    return error;
                }
            }
        }
        ,onRowCountChanged : function (provider, count) {
            $('#detailAdveWrdGridCnt').text(count);
        }
    }
    // 홍보문구 초기화
