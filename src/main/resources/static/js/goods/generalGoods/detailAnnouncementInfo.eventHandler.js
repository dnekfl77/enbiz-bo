var detailAnnouncementInfoMsg = x2coMessage.getMessage({
...
$.namespace('detailAnnouncementInfo.eventhandler');
detailAnnouncementInfo.eventhandler = {
    // 초기화
    init : function () {
    ...
    ,bindButtonEvent : function () {
        ...
        $(document).on('change', 'input[name=safeCertiNeedYn]', function(){
            that.resetSafeCertiInfo( $(this).val() )
        });

    }
    // 안전인증정보 초기화
    ,resetSafeCertiInfo : function ( safeCertiNeedYn ) {
    ...
    ,addSafeCertiInfo : function ( position ) {
        ...
        if( safeCertiInfoCnt === 5 ){
            alert(detailAnnouncementInfoMsg.maxSafeCertiItemMsg);
            return;
        }
        ...
        newSafeCertiInfo.find('.btn_remove_safe_certi_info').bind('click', function () {
            ...
            if(safeCertiInfoCnt === 1){
                alert(detailAnnouncementInfoMsg.minSafeCertiItemMsg);
                return;
            }
            $(this).parent().remove();
        });
    ...
    ,validationCheck : function () {
        ...
        if ( $('#announcementInfo :radio[name=safeCertiNeedYn]:checked').val() === undefined ) {
            alert(detailAnnouncementInfoMsg.noSafeCertiNeedYnMsg);
        ...
        if ( $('#announcementInfo :radio[name=safeCertiNeedYn]:checked').val() === 'Y' ) {
            ...
            safeCertiInfoList.each( function( index, data ) {
                ...
                var safeCertiOrnNm  = $(data).find('input[name=safeCertiOrnNm]'); // 안전인증기관명(필수값X)
                ...
                if( safeCertiGbCd.val() === '' ) {
                    alert(detailAnnouncementInfoMsg.noSafeCertiGbCdMsg);
                    safeCertiGbCd.focus();
                    isEmpty = true;
                    return false;
                }
                ...
                if( safeCertiNo.val() === '' ) {
                    alert(detailAnnouncementInfoMsg.noSafeCertiNoMsg);
                    safeCertiNo.focus();
                    isEmpty = true;
                    return false;
                }
                ...
                if( aplyStrDt.val() === '' || aplyEndDt.val() === '' ) {
                    alert(detailAnnouncementInfoMsg.noAplyDateMsg);
                    aplyStrDt.focus();
                    isEmpty = true;
                    return false;
                }
    ...
    ,setAnnouncementInfo : function ( announcementInfoData ) {
        ...
        var that = this;
        var prGoodsBase = announcementInfoData.prGoodsBase;
        var safeCertiTgtYn = announcementInfoData.prGoodsBase.safeCertiTgtYn;     // 상품 - 안전인증대상여부
        var safeCertiNeedYn = announcementInfoData.prGoodsBase.safeCertiNeedYn;   // 표준카테고리 - 안전인증필요여부

        // 상품고시품목코드 세팅
        $('#goodsNotiLisartCd').val( convertNullToEmptyString(prGoodsBase.goodsNotiLisartCd) );
        $('#goodsNotiLisartCd').addClass('disabled').attr('disabled', true);

        var prGoodsItemInfoList = announcementInfoData.prGoodsItemInfoList;
        if( prGoodsItemInfoList != null && prGoodsItemInfoList.length > 0 ){
            prGoodsItemInfoList.forEach( function ( data ) {
        ...
        // 안전인증정보 세팅
        $('#announcementInfo :radio[name=safeCertiNeedYn][value='+safeCertiTgtYn+']').prop('checked', true);    // 안전인증대상여부 초기화
        that.resetSafeCertiInfo(safeCertiTgtYn);                                                                // 안전인증정보초기화

        var prGoodsSafeCertiHistList = announcementInfoData.prGoodsSafeCertiHistList;
        if (prGoodsSafeCertiHistList != null && prGoodsSafeCertiHistList.length > 0 ) {
            var newSafeCertiInfo = $('div[name=newSafeCertiInfo]');
            $.each( prGoodsSafeCertiHistList, function ( index, data ) {
                ...
                if( (index + 1) === prGoodsSafeCertiHistList.length ) return false;
                newSafeCertiInfo = that.addSafeCertiInfo( newSafeCertiInfo );
            });
        }

        // 표준카테고리의 안전인증필요여부값이 'N'일 경우 상품의 안전인증대상여부 변경 가능
        if ( safeCertiNeedYn === 'N' ) { // 변경가능
            $('input[name=safeCertiNeedYn]').attr('disabled',false);
        }
    }
}