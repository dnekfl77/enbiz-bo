$.namespace("connectTabBrandGrid.eventhandler");
connectTabBrandGrid.eventhandler = {
    // 초기화
    init : function () {
        this.gridLoading();
    ...
    bindButtonEvent : function(){
        ...
        // 그리드 브랜드 추가 버튼
        $("#btn_brandGrid_add").click(function() {
            self.onAdd();
        });

        // 그리드 행삭제 버튼
        $("#btn_brandGrid_remove").click(function() {
            self.grid.cancel();
            self.onDelete();
        });

        // 그리드 변경 취소 버튼
        $("#btn_brandGrid_reset").click(function() {
            self.onReset();
        });

        // 그리드 저장 버튼
        $("#btn_brandGrid_save").click(function() {
            self.onSave();
        });

        // 그리드 전시기간 일괄변경 버튼
        $("#btn_brandGrid_updateDispDateList").click(function() {
            self.onTermChange();
        });
    },
    ...
    onAdd : function() {
        ...
        popCommon(pin, POP_DATA, this.brandPopupCallback.bind(this));
    },

    brandPopupCallback : function(e){
        this.grid.commit();
        var resultData = JSON.parse(e.data);
        ...
        for(var i = 0; i<rowCount; i++){
            for(var j = 0; j<resultData.length; j++) {
                if(resultData[j].brandNo == this.grid.getValue(i , "conrContNo") ) { // 추가된 상품과 원래 그리드에 있던 상품 비교
        ...
        for(var i=0; i<contents.length; i++) {
            var param = {
                brandNo : contents[i].brandNo
            }
            common.Ajax.sendRequestSync(
                 "POST"
                 ,_baseUrl + "display/displayConnectMgmt.getBrandDetail.do"
                 ,param
                 ,function(obj) {
       ...
       var self = this;
        ...
        for(var i=0; i<dataList.length; i++) {
            ...
            dataList[i].conrContNo = dataList[i].brandNo;
            dataList[i].dispSeq = Number(maxDispSeq) + i;
            dataList[i].dispYn = "Y";
            dataList[i].movFrmeCd = "";
