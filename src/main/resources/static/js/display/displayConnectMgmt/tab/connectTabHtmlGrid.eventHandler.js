$.namespace("connectTabHtmlGrid.eventhandler");
connectTabHtmlGrid.eventhandler = {
    // 초기화
    init : function () {
        this.gridLoading();
    ...
    bindButtonEvent : function(){
        ...
        // 그리드 HTML 추가 버튼
        $("#btn_htmlGrid_add").click(function() {
        ...
        // 그리드 행삭제 버튼
        $("#btn_htmlGrid_remove").click(function() {
            self.grid.cancel();
            self.onDelete();
        });

        // 그리드 저장 버튼
        $("#btn_htmlGrid_save").click(function() {
            self.onSave();
        });
    },
    ...
    onAdd : function(state, dispCtgNo, conrNo, conrTgtNo, conrTgtCd, conrContNo) {
        ...
        if(state == "I"){
            var POP_DATA = {
                url:  _baseUrl + 'display/displayConnectMgmt.connectHtmlPopup.do'
                , winName: 'connectHtmlInsert'
                , title: 'HTML 전시 등록'
                , _title: 'HTML 전시 등록'
                , left: 20
                , top: 20
