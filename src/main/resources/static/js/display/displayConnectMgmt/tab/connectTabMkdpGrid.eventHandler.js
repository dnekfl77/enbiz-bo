$.namespace("connectTabMkdpGrid.eventhandler");
connectTabMkdpGrid.eventhandler = {
    // 초기화
    init : function () {
        this.gridLoading();
    ...
    bindButtonEvent : function(){
        ...
        // 그리드 기획전 추가 버튼
        $("#btn_mkdpGrid_add").click(function() {
            self.onAdd();
        });

        // 그리드 행삭제 버튼
        $("#btn_mkdpGrid_remove").click(function() {
            self.grid.cancel();
            self.onDelete();
        });

        // 그리드 변경 취소 버튼
        $("#btn_mkdpGrid_reset").click(function() {
            self.onReset();
        });

        // 그리드 저장 버튼
        $("#btn_mkdpGrid_save").click(function() {
            self.onSave();
        });

        // 그리드 전시기간 일괄변경 버튼
        $("#btn_mkdpGrid_updateDispDateList").click(function() {
            self.onTermChange();
        });
    },
    ...
    onAdd : function() {
        ...
        popCommon(pin, POP_DATA, this.mkdpPopupCallback.bind(this));
    },

    mkdpPopupCallback : function(e){
        this.grid.commit();
        var resultData = JSON.parse(e.data);
        ...
        for(var i = 0; i<rowCount; i++){
            for(var j = 0; j<resultData.length; j++) {
                if(resultData[j].mkdpNo == this.grid.getValue(i , "conrContNo") ) { // 추가된 상품과 원래 그리드에 있던 상품 비교
       ...
       var self = this;
        ...
        for(var i=0; i<dataList.length; i++) {
            ...
            dataList[i].conrContNo = dataList[i].mkdpNo;
			dataList[i].mkdpTypCd = dataList[i].dispGrpTypNm;
			dataList[i].dispStat = dataList[i].dispStatName;
