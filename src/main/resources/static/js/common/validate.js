...
function isNotValidPassword(passField, confirmField) {
	...
	if(isOutOfRange(passField, 4, 10, "비밀번호는 4~10자 사이의 숫자 및 영문 대소문자로만 기입해 주세요!")) return true;
	if(isOutOfRange(confirmField, 4, 10, "비밀번호는 4~10자 사이의 숫자 및 영문 대소문자로만 기입해 주세요!")) return true;
...
function isNotValidPasswordValue(idObj, pwObj) {
    ...
    if(pwStr.length < 8) {
        alert("비밀번호는 8자리 이상으로 기입해 주세요.");
        pwObj.focus();
        return true;
    }

    if(!pwStr.match(/([a-zA-Z0-9].*[!,@,#,$,%,^,&,*,?,_,~])|([!,@,#,$,%,^,&,*,?,_,~].*[a-zA-Z0-9])/)) {
        alert("비밀번호는 영문, 숫자, 특수문자의 조합으로 입력해주십시오.");
        pwObj.focus();
        return true;
    }

    if(pwStr.indexOf(idStr) >= 0) {
        alert("ID는 패스워드에 포함되어서는 안됩니다.")
        pwObj.focus();
        return true;
    }
    ...
    for(var i=0; i < pwStr.length-4; i++) {
        if(idStr.indexOf(pwStr.substring(i, i+4)) >= 0) {
            alert("ID와 4자리 이상 유사한 비밀번호는 사용할 수 없습니다..")
            pwObj.focus();
            return true;
        }
...
$.fn.formValidate = function() {
	...
	_Input.each(function(index, input){
		...
		if(inputCheck == 'string' && $.trim($Input.val()) == ''){
			alert($Input.attr('title') + _altBlankMsg);
			$Input.focus();
			result = false;
			return false;
		}
