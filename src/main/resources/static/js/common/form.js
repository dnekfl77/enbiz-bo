...
w.getElementValue = function(obj){
    ...
    if(typeof(tname) == "undefined") {
        ...
        if(clsid == Clsid_MxMaskEdit || clsid == Clsid_MxTextArea){
            ...
            window.alert("[ "+obj + " ]의 Value는 Object 입니다. ");
            returnStr = defaultVal;
        }
    }else{
        returnStr = defaultVal;
...
w.MakeFormPopup = function(url, winName, features){
    if(typeof(d.popupform) != "undefined"){
        d.body.removeChild(d.popupform);
    }

    var newForm = d.createElement("<form name=popupform method=post target="+winName+"></form>");
    d.body.insertBefore(newForm);

    this.put = function(key, value){
        newForm.insertBefore(w.genDomInput(key, value));
    };
    ...
    this.submit = function(){
        var popup = w.open("about:blank", winName, features);
        if(popup === null){
            window.alert('popup window is null');
        }else{
            popup.focus();
