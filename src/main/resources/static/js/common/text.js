 ...
 * 입력 Byte 수 제한(UTF-8)
 * @param obj : $(this)
 * @param limitByte : 제한 byte 수
 * @param markId : 입력 byte 수 표시할 태그 ID
 * UTF-8 : 알파벳/숫자 -> 1byte, 한글 -> 3bytes
 */
function byteLimitCheck ( obj, limitByte, markId){
	...
	if ( markId === '') return;
	$('#'+markId).text(resultBytes);
};

/**
 * 입력 문자의 Byte 값 계산(UTF-8)
 * @param value
 * @returns {number}
 */
