	...
	var nMaxTotalImageSize = 25*1024*1024;
	...
	function generalUpload(){
		var options = {
		    url : "/common/file/uploadImgEditor.do",
		    datatype:"json",
		    success: function(data){
//		    	var jsonData = $.parseJSON(data);
		    	var jsonData = data;
		    	makeArrayFromString(jsonData.Val, 1);
		    }
		  };
		  $("#editor_upimage").ajaxSubmit(options);
	}
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
			alert("전체 이미지 용량이 25MB를 초과하여 등록할 수 없습니다. \n\n (파일명 : "+sFileName+", 사이즈 : "+sFileSize+")");
		} else {
			//이미지 정보 저장							
    ...
    function html5Upload() {	
    	...
    	sUploadURL= "/common/file/uploadImgEditor.do"; 	//upload URL
    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
	    			...
	    			uploadFile(tempFile,sUploadURL);
	    			k += 1;
	    		}
	    	}catch(e){}
    		tempFile = null;
    ...
    function uploadFile(tempFile, sUploadURL){
    	var formData = new FormData();
    	formData.append('UPLOAD_FILE',tempFile);
    	$.ajax({
	        url : sUploadURL,
	        processData : false,
	        contentType : false,
	        ...
	        type : 'POST',
    		success:function(data){
    			makeArrayFromString(data.Val, nImageFileCount);
    		},
    		error:function(){
...
*/    
    function makeArrayFromString(sResString, fileCnt){
    	var	aTemp = [],
    		aSubTemp = [],
    		htTemp = {}
    		aResultleng = 0;
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/admin/test/uploadFileTest.do',	//샘플 URL입니다.
 	        sCallback : '/admin/prod/regProd.do',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) {alert("success");
 	    		// alert("success");
 	    	...
 	    	error : function(oCustomEvent) {alert("eeee");
 	    		//업로드가 실패했을 때 발생
 	    		//oCustomEvent(이벤트 객체) = {
